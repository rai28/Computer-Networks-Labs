...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        fgets(respMessage, 20, stdin);
        // close the connection if respMessage is exit
        if (strcmp(respMessage, "exit\n") == 0)
        {
            printf("Closing connection\n");
            //send server closing message
            write(newsockfd, "Server closing connection\n", 25);
            close(newsockfd);
            break;
        }
